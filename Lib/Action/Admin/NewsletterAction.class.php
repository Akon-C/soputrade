...
class NewsletterAction extends AdminCommAction{
	...
	public function doSend(){
		...
		switch ($_POST['type']){
			...
			self::$Model=D('Shippingadress');
			$model=D('Members');
			$list=self::$Model-query("select isNewseletter from __TABLE__ where isNewseletter is not null");
			foreach ($list as $k=>$v){
				if($v['isNewseletter']==1){
					$sendTo[]=$model->where('id='.$v['id'])->getField('email');
				}else{
					$sendTo[]=$v['isNewseletter'];
				}
			}
			break;
